...
import { FindConditions, FindManyOptions, getConnection, getRepository, ObjectID, Repository, SelectQueryBuilder } from 'typeorm';
...
export type QueryCallback<T> = (qb: SelectQueryBuilder<T>) => void;
...
export class UniversalService<Entity, ID = any> {
  ...
  constructor(public readonly entity: ClassType<Entity>) {
    this._repo = getRepository(entity);
  ...
  async getAll<T = Entity>(voClass?: ClassType<T>, groups?: string[]) {
    const records = await this.repo.find(this.hasProperty('isDel') ? { where: { isDel: false } } : undefined);
    if (voClass) {
  ...
  async readByIds(ids: ID[], options?: FindManyOptions<Entity>) {
    if (this.hasProperty('isDel')) {
      return this._repo.findByIds(ids, _.assign({}, { where: { isDel: false } }, options));
    } else {
      return this._repo.findByIds(ids, options);
    }
  }

  async readById(ids: ID, options?: FindManyOptions<Entity>) {
    if (this.hasProperty('isDel')) {
      return this._repo.findOne(ids, _.assign({}, { where: { isDel: false } }, options));
    } else {
      return this._repo.findOne(ids, options);
    }
  }

  async update(criteria: string | number | Date | ObjectID | string[] | number[] | Date[] | ObjectID[] | FindConditions<Entity>, partialEntity: QueryDeepPartialEntity<Entity>, autoGenerateData = true) {
    if (autoGenerateData) {
      const autoGen = (entity: any) => {
  ...
  async remove(criteria: string | number | Date | ObjectID | string[] | number[] | Date[] | ObjectID[] | FindConditions<Entity>) {
    if (this.hasProperty('isDel')) {
    ...
    if (search.search && !_.isNil(options.searchColumns)) {
      ...
      search.search.split(' ').forEach((cla, idx) => {
        options.searchColumns!.forEach((column) => {
          _cla.push(`_tb.${column} like :${column}${idx}`);
          conditions[`${column}${idx}`] = `%${cla}%`;
        });
      });
      if (!_.isEmpty(_cla)) {
