...
async function generateRepositories() {
  ...
  try {
    ...
    const configs: string[] = [
      '-h', hostname,                                       // 数据库主机 / Database host
      '-d', pathname.substring(1),                          // 数据库名 / Database name
      '-p', port,                                           // 端口 / Port
      '-u', username,                                       // 用户名 / Username
      '-x', password,                                       // 密码 / Password
      '-e', 'mysql',                                        // 数据库类型 / Database type
      '-o', `${cfg.output || './src'}/repositories`,       // 输出目录 / Output directory
      '--noConfig',                                         // 不使用配置文件 / Don't use config file
      '--cf', 'pascal',                                     // 类文件名格式: PascalCase / Class file naming: PascalCase
      '--ce', 'pascal',                                     // 类名格式: PascalCase / Class naming: PascalCase
      '--cp', 'camel',                                      // 属性名格式: camelCase / Property naming: camelCase
